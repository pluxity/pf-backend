...
class ChatPromptBuilder(
    ...
    private fun buildClarifyContext(history: List<Message>): String {
        val entries = history.filter { it.role == "system" && it.content.startsWith("---") }
        if (entries.isEmpty()) return ""

        val lastEntry = entries.last().content
    ...
    private fun appendHistory(
        ...
        val entries = history.filter { it.role == "system" && it.content.startsWith("---") }
        if (entries.isEmpty()) return basePrompt
        ...
        return buildString {
            ...
            entries.forEach { appendLine(it.content) }
            if (isClarifyPending) {
                appendLine()
