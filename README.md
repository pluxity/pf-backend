...
│   ├── auth/           # JWT (nimbus-jose-jwt), Security, User, Role, Permission
│   ├── file/           # S3, Local 파일 업로드
│   ├── messaging/      # WebSocket, STOMP
│   └── test-support/   # 테스트 헬퍼, Dummy 팩토리
├── apps/
│   ├── safers/         # Safers 앱 (CCTV, Event, Site 등)
│   └── yongin-platform/# 용인 플랫폼 앱 (CCTV + 즐겨찾기 등)
└── gradle/
...
### Kotlin JDSL (Spring Boot 4 호환)

Kotlin JDSL을 통한 타입 안전한 JPQL 쿼리 빌더. Spring Boot 4에서는 `spring-data-jpa-boot4-support` 모듈을 사용하며,
`EntityManager` bean이 자동 등록되지 않으므로 `KotlinJdslConfig`에서 `SharedEntityManagerCreator`로 수동 등록:

```kotlin
// common/core의 KotlinJdslConfig.kt
@Configuration
...
Repository에서 `KotlinJdslJpqlExecutor`를 확장하여 JDSL 쿼리 사용:
...
// Service에서 사용
val page = eventRepository.findPageNotNull(pageable) {
...
- `nimbus-jose-jwt` 기반 JWT 토큰 발급/검증
...
| 앱               | 고유 기능                                                  |
|-----------------|--------------------------------------------------------|
| safers          | CCTV (Site 연관), Event, Site, Hibernate Spatial + JTS   |
| yongin-platform | CCTV + 즐겨찾기(Bookmark)                                  |

> CCTV 도메인은 앱별로 요구사항이 달라 각 앱에 독립적으로 구현되어 있습니다.
> safers는 Site 기반 CCTV 관리, yongin-platform은 즐겨찾기 기능을 포함합니다.
